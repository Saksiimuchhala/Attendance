...
def add():
    ...
    # Video writer setup
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    video_filename = f"static/videos/{newusername}_{newuserid}.avi"
    out = cv2.VideoWriter(video_filename, fourcc, 20.0, (640, 480))
...
def attendance():
    try:
        ...
        if os.path.exists(csv_file):
            ...
            return render_template('attendance.html', attendance_data=attendance_data, total_registered=total_registered(), datetoday2=datetoday2)
        return render_template('attendance.html', attendance_data=[], total_registered=total_registered(), datetoday2=datetoday2)
    except Exception as e:
